...
function addCylinder(t)
    ...
    if t.sector ~= nil then
        segment = not t.sector
    end
    local l,am
    if rl then
        l = vec3(0,0,0)
    ...
    da = math.rad(da)/n
    if segment then
        sm = sc + math.cos(sa)*si/2 + math.sin(sa)*sj/2 + math.cos(ea)*si/2 + math.sin(ea)*sj/2
        em = ec + math.cos(sa)*ei/2 + math.sin(sa)*ej/2 + math.cos(ea)*ei/2 + math.sin(ea)*ej/2
    end
    o = (sm/2 + sc/2 + math.cos((sa+ea)/2)*si/2 + math.sin((sa+ea)/2)*sj/2 + em/2 + ec/2 + math.cos((sa+ea)/2)*ei/2 + math.sin((sa+ea)/2)*ej/2)/2
    ...
    cnrs[2] = {sm, sc+cs*si - sn*sj, ec+cs*ei - sn*ej, em}
    ...
    if ends%2 == 1 then
        to.x = to.x + ts.x
        p = __doCone(m,p,n,o,sm,u,tw,col,to,ts,f,closed,l,am)
    end
    if ends >= 2 then
        to.x = to.x + ts.x
        p = __doCone(m,p,n,o,em,v,tw,col,to,ts,f,closed,l,am)
    end
    if ret then
        return m,ip, p
    else
